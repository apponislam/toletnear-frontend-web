      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 24px' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: 64 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 2 }} className="hidden md:flex">
            {navLinks.map(link => {
              ...
              return (
                <Link key={link.label} href={link.href} style={{ padding: '7px 16px', borderRadius: 7, border: 'none', background: isActive ? '#EBF2FF' : 'none', cursor: 'pointer', fontSize: 14, fontWeight: 500, color: isActive ? '#1A4F9E' : '#374151', transition: 'all 0.15s', textDecoration: 'none' }}>
                  {link.label}
                </Link>
              )
            })}
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }} className="hidden md:flex">
            {isLoggedIn ? (
              <div style={{ position: 'relative' }}>
                ...
                {profileOpen && (
                  <div style={{ position: 'absolute', right: 0, top: '110%', backgroundColor: '#fff', borderRadius: 12, boxShadow: '0 8px 30px rgba(0,0,0,0.12)', border: '1px solid #E2E8F0', minWidth: 190, overflow: 'hidden', zIndex: 100 }}>
                    <Link href={dashHref} onClick={() => setProfileOpen(false)} style={{ display: 'block', width: '100%', textAlign: 'left', padding: '11px 16px', border: 'none', background: 'none', cursor: 'pointer', fontSize: 14, color: '#374151', textDecoration: 'none' }}>Dashboard</Link>
                    <Link href="/messaging" onClick={() => setProfileOpen(false)} style={{ display: 'block', width: '100%', textAlign: 'left', padding: '11px 16px', border: 'none', background: 'none', cursor: 'pointer', fontSize: 14, color: '#374151', textDecoration: 'none' }}>Messages</Link>
                    <Link href="/post-property" onClick={() => setProfileOpen(false)} style={{ display: 'block', width: '100%', textAlign: 'left', padding: '11px 16px', border: 'none', background: 'none', cursor: 'pointer', fontSize: 14, color: '#374151', textDecoration: 'none' }}>Post Property</Link>
              ...
              <>
                <Link href="/login" style={{ padding: '8px 20px', borderRadius: 8, border: '1.5px solid #1A4F9E', background: '#fff', color: '#1A4F9E', cursor: 'pointer', fontSize: 14, fontWeight: 500, textDecoration: 'none' }}>Login</Link>
                <Link href="/register" style={{ padding: '8px 20px', borderRadius: 8, border: 'none', background: '#1A4F9E', color: '#fff', cursor: 'pointer', fontSize: 14, fontWeight: 600, textDecoration: 'none' }}>Register</Link>
              </>
            )}
          </div>
        ...
        {mobileOpen && (
          <div style={{ borderTop: '1px solid #E2E8F0', padding: '12px 0 16px' }} className="md:hidden">
            ...
            {isLoggedIn && (
              <Link href={dashHref} onClick={() => setMobileOpen(false)} style={{ display: 'block', width: '100%', textAlign: 'left', padding: '10px 4px', border: 'none', background: 'none', cursor: 'pointer', fontSize: 15, color: '#374151', textDecoration: 'none' }}>Dashboard</Link>
            )}
            <div style={{ borderTop: '1px solid #E2E8F0', marginTop: 8, paddingTop: 10, display: 'flex', gap: 8 }}>
              <Link href="/login" onClick={() => setMobileOpen(false)} style={{ flex: 1, padding: '9px', borderRadius: 7, border: '1.5px solid #1A4F9E', background: '#fff', color: '#1A4F9E', cursor: 'pointer', fontSize: 14, textAlign: 'center', textDecoration: 'none' }}>Login</Link>
              <Link href="/register" onClick={() => setMobileOpen(false)} style={{ flex: 1, padding: '9px', borderRadius: 7, border: 'none', background: '#1A4F9E', color: '#fff', cursor: 'pointer', fontSize: 14, textAlign: 'center', textDecoration: 'none' }}>Register</Link>
            </div>
          </div>
        )}
      </div>
